...
# Pythonanywhere runs daily, so conditional only runs code when the day is the 2 for the monthly report and 15 for the mid-month report
if int(datetime.date.today().day) == 2 or int(datetime.date.today().day) == 15:
    if int(datetime.date.today().day) == 15:
        sys.argv.append('mid_month')
    ...
    for data in response['results']:
        ...
        sub = 1
        month_string = (datetime.date.today().replace(day=1) - datetime.timedelta(days=1)).strftime("%B")
        if len(sys.argv) > 1:
            if sys.argv[1] == "mid_month":
                sub = 0
                month_string = (datetime.date.today().replace(day=1)).strftime("%B")
            else:
